...
public class TransferView extends JPanel {
    ...
    private JTextField txtNoRekening, txtAmount;
    private JButton btnTransfer, btnBack;
    private TransactionController transactionController;
    private DashboardView parentDashboard;

    public TransferView(Card card, Account account, DashboardView parentDashboard) {
        this.card = card;
    ...
    private void initComponents() {
        ...
        // Header - NO EMOJI
        JLabel lblTitle = new JLabel("Transfer Uang");
        lblTitle.setFont(new Font(UIConstants.FONT_FAMILY, Font.BOLD, 28));
        lblTitle.setForeground(UIConstants.TEXT_PRIMARY);
        lblTitle.setAlignmentX(Component.CENTER_ALIGNMENT);

        JLabel lblSubtitle = new JLabel("Kirim uang ke rekening lain");
        ...
        // Info panel
        JPanel infoPanel = UIComponents.createInfoPanel();
        infoPanel.setLayout(new BoxLayout(infoPanel, BoxLayout.Y_AXIS));
        infoPanel.setAlignmentX(Component.LEFT_ALIGNMENT);

        JLabel lblInfo = new JLabel("Informasi:");
        lblInfo.setFont(UIConstants.FONT_BODY_BOLD);
        lblInfo.setForeground(UIConstants.INFO);
        lblInfo.setAlignmentX(Component.LEFT_ALIGNMENT);

        JLabel lblInfo1 = new JLabel("• Pastikan nomor rekening tujuan benar");
        ...
        JLabel lblInfo2 = new JLabel("• Transfer diproses real-time");
        ...
        // No Rekening
        JLabel lblNoRek = new JLabel("Nomor Rekening Tujuan");
        lblNoRek.setFont(UIConstants.FONT_BODY_BOLD);
        lblNoRek.setForeground(UIConstants.TEXT_PRIMARY);
        lblNoRek.setAlignmentX(Component.LEFT_ALIGNMENT);

        txtNoRekening = UIComponents.createTextField();
        txtNoRekening.setMaximumSize(new Dimension(Integer.MAX_VALUE, 45));
        txtNoRekening.setAlignmentX(Component.LEFT_ALIGNMENT);

        JLabel lblHint1 = new JLabel("10 digit nomor rekening");
        ...
        formCard.add(lblNoRek);
        formCard.add(Box.createVerticalStrut(8));
        formCard.add(txtNoRekening);
        formCard.add(Box.createVerticalStrut(5));
        formCard.add(lblHint1);
        formCard.add(Box.createVerticalStrut(20));

        // Amount
        JLabel lblAmount = new JLabel("Jumlah Transfer");
        ...
        JLabel lblHint2 = new JLabel("Contoh: 100000, 500000");
        ...
        btnTransfer = UIComponents.createPrimaryButton("Transfer");
    ...
    private void handleTransfer() {
        String noRekening = txtNoRekening.getText().trim();
        String amountStr = txtAmount.getText().trim();

        if (noRekening.isEmpty() || amountStr.isEmpty()) {
        ...
        try {
            ...
            int confirm = JOptionPane.showConfirmDialog(parentDashboard,
                "Transfer ke rekening " + noRekening + "\nJumlah: Rp " + String.format("%,d", amount) + "\n\nLanjutkan?",
                "Konfirmasi",
                JOptionPane.YES_NO_OPTION);

            if (confirm == JOptionPane.YES_OPTION) {
                btnTransfer.setEnabled(false);
                btnTransfer.setText("Memproses...");

                SwingWorker<String, Void> worker = new SwingWorker<String, Void>() {
                    @Override
                    protected String doInBackground() {
                        return transactionController.transfer(account.getId(), noRekening, amount);
                    }
